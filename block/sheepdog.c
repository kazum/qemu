...
static coroutine_fn void reconnect_to_sdog(void *opaque)
{
    ...
    dprintf("resent all the failed reqs\n");
}
...
static void coroutine_fn aio_read_response(void *opaque)
{
    ...
    if (ret < sizeof(rsp)) {
        error_report("failed to get the header, %d, %s", ret, strerror(errno));
        goto err;
    }
    ...
    dprintf("read aio %p, (oid %lx, offset %lx, len %x), %s\n", aio_req,
            aio_req->oid, aio_req->offset, aio_req->data_len,
